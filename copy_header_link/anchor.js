...
document.addEventListener("DOMContentLoaded", function() {
  ...
  function createSvgIcon(viewBoxValue, innerHtmlValue, displayValue, className) {
    ...
    svgIcon.style.display = displayValue; // Устанавливаем свойство display в момент создания: none — для скрытия кнопки, block — для отображения.
    svgIcon.setAttribute('class', className); // Устанавливаем класс для SVG-элемента
  ...
  if (document.querySelector('h2[id]')) {
    ...
    headersList.forEach((item) => {
      item.style.display = 'flex'; // Задаем для заголовка свойство display. Вынужденная мера, т.к. Flare не знает flexbox.

      const copyBtnAttrs = createSvgIcon("0 0 24 24", copyIconInnerTag, 'none', 'copy_btn');
      ...
      copyBtn.addEventListener('click', (e) => { // Навешиваем слушатель клика по кнопке.
        ...
        if (header) {
          ...
          copyIcon.style.display = 'none'; // Скрываем кнопку.
          const checkIconAttrs = createSvgIcon("0 0 32 32", checkIconInnerTag, 'block', 'check_icon');
      ...
      item.addEventListener('mouseenter', () => { // Проверяем, когда курсор наведен на заголовок.
        copyIcon.style.display = 'block'; // Делаем кнопку видимой.
      });
